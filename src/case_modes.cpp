...
std::vector<std::pair<fs::path, std::string>> generate_sequential_names(const std::vector<std::pair<std::string, fs::path>>& folder_names, const std::string& prefix) {
    std::vector<std::pair<fs::path, std::string>> sequential_names;
    int counter = 1;
    ...
    for (const auto& [folder_name, folder_path] : folder_names) {
        ...
        std::string new_name = prefix.empty() ? ss.str() : (prefix + "_" + ss.str());

        sequential_names.emplace_back(folder_path, new_name);
        counter++;
    }
