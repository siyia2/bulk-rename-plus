...
std::string move_date_to_back(const std::string& file_string) {
    ...
    size_t extension_position = file_string.find_last_of('.');
    
    if (date_position != std::string::npos && date_position + 9 <= file_string.length()) {
        // Extract the date sequence
        std::string date_seq = file_string.substr(date_position, 8);

        // Construct the new filename with the modified date sequence at the back and extension
        return file_string.substr(0, date_position) + file_string.substr(date_position + 9, extension_position - date_position - 9) + "_" + date_seq +
               ((extension_position != std::string::npos) ? file_string.substr(extension_position) : "");
    } else {
